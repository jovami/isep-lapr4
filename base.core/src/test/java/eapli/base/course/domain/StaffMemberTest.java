...
class StaffMemberTest {
    ...
    void setUp() {
        ...
        SystemUser user = createSystemUser("userName","Password1","First","Last","user@email.com",BaseRoles.TEACHER);
    ...
    public Teacher createTeacher(SystemUser user, String acronym, String dateOfBirth, String fullName, String shortName, String taxPayerNumber) {
        TeacherBuilder teacherBuilder = new TeacherBuilder().withSystemUser(user).withAcronym(acronym).withDateOfBirth(dateOfBirth)
                .withFullName(fullName).withTaxPayerNumber(taxPayerNumber).withShortName(shortName);
        return teacherBuilder.build();
    }
    public SystemUser createSystemUser(String userName, String password, String firsName, String lastName, String email, final Role ...roles) {
