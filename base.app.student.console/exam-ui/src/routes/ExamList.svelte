<script lang="ts">
  ...
  const examList = async () => {
    const res = await fetch(
      "http://localhost:8090/api/examtaking/formative-exams"
    );
    const body = await res.json();

    if (res.ok) return body as ExamInfo[];
...
<div class="text-gray-400 body-font bg-gray-900">
  <div class="container px-5 py-24 mx-auto">
              ...
              <button on:click={() => (selected = exam)}>
                ...
                >
                  ...
                  <h2 class="text-lg text-white font-medium title-font mb-2">
                    '{exam.examName}' &lt;- empty lol &mdash; {exam.courseName}
                  </h2>
                  <p class="leading-relaxed text-base">Bottom Text</p>
                </div>
              </button>
            </div>
          {/each}
    ...
    <div
      ...
      on:click={() => {examStore.set(selected)}}
    >
      <a href="/take" use:link>Take</a>
    </div>
